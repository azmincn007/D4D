...
function Mobilessingle() {
  ...
  const contentcardres = [
    { img: re, title: "Lulu Eranakulam", content: "Sprawling, residential Ernakulam is known for Marine Drive, a busy ..." },
    { img: re, title: "Lulu Eranakulam", content: "Sprawling, residential Ernakulam is known for Marine Drive, a busy ..." },
    { img: re, title: "Lulu Eranakulam", content: "Sprawling, residential Ernakulam is known for Marine Drive, a busy ..." },
    { img: re, title: "Lulu Eranakulam", content: "Sprawling, residential Ernakulam is known for Marine Drive, a busy ..." },
    { img: re, title: "Lulu Eranakulam", content: "Sprawling, residential Ernakulam is known for Marine Drive, a busy ..." },
  ...
  return (
    <div className="mobilessingle">
      ...
      <div className="py-6 px-5 pb-16">
        {renderData.map((mobile, index) => (
          <div key={index} className="singlecarddetails mb-4 bg-[#F1F1F1] pt-4 px-4 rounded-[20px] font-inter Mobile:px-2 Mobile:py-2">
            <div className="topsingle flex">
              ...
              <div className="rightsingle px-2 Tab:w-[50%]">
                <div className="righthead">
                  ...
                  <div className="hidden Tab:block py-1 mb-2">
                    <p className="text-[12px] text-[#1F1F1F] LgMobile2:text-[8px]">The Walk at - Jumeirah Beach Residence - Dubai - United Arab Emirates</p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap">
                    {mobile.branches.map((branch, i) => (
                        <div 
                        className='px-4 rounded-[100px] border-[1px] border-black py-2 mr-2 mt-1 text-12px Mobile:mr-1 branchsingle' 
                        key={i}
                      >
                        <span>{branch}</span>
                      </div>
            ...
            <div className="bottomsingle flex justify-between items-center  Tab:hidden">
              <div className="flex  py-4 items-center justify-between  w-[322px]">
                <div className="text-[18px]">{mobile.title}</div>
                <div className="bg-yellow rounded-[100px] px-4 py-2">{mobile.price}</div>
              </div>

              <Link to={"/flyer"} state={{ source: "mobile" }}>
                <div className=" bg-yellow rounded-[100px] px-4 py-2 font-semibold  flex items-center h-[40px] justify-center">GO TO FLYERS</div>{" "}
              </Link>
            </div>
            ...
            <div className="hidden bottomsingle  justify-between items-center py-2 Tab:block">
              <div className="flex  py-2 items-center justify-between  w-[100%]">
                ...
                <Link to={"/flyer"} state={{ source: "mobile" }}>
                  <div className=" butsng bg-yellow rounded-[100px] px-2 py-2 font-semibold  flex items-center h-[40px] justify-center Mobile:text-[12px] Mobile:h-[34px] ">
                    GO TO FLYERS
                  </div>
                </Link>
              </div>
        ...
        <div>
          <div className="font-semibold text-[20px] py-2 font-inter">{source === "restaurant" ? "Similar Food Items" : "Similar Products"}</div>{" "}
          <div className="contentscards">
            <div className="cardcontainermobsingle ">
              {renderData2.map((obj, index) => (
                <Homecards key={index} img={obj.img} title={obj.title} content={obj.content} />
              ))}
            </div>
          </div>
        </div>
